...
打开后是这样，把第一个`false`改成`true`(webui那个可选)，其他的基本不用管。**改完重启bot**。
```yaml
enable: false            # 默认关闭。开启后会在桌面显示 Live2D 桌宠（或群内 /live2d on）
webui_enable: false         # 开启后浏览器访问 http://localhost:5007/live2dchat 即网页版桌宠（与 WebUI、桌宠共用对话后端/上下文）
...
expression:                # 表情/动作控制：对话后用一个快速模型把回复判定为某种“情绪/场景”，再按下方索引变脸/播动作
  ...
  base_url: ""             # 留空则复用 mai_reply 的 trigger_llm 端点
