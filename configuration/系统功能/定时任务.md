# 定时任务
定时任务功能尚未开发完成，请等待后续更新。
## 定时任务
```
#可用任务名["每日天文","bing每日图像", "单向历", "bangumi", "nightASMR", "摸鱼人日历", "新闻", "免费游戏喜加一", "早安", "晚安", "午安"]
/cron add 任务名
/cron remove 任务名
```
对于私聊早安天气播报和晚安提醒，可在配置文件中设置。
## 配置
配置文件`run/scheduled_tasks/config.yaml`,webui中则为`定时任务.config`
```yaml
